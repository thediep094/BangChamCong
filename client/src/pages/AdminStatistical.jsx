...
const AdminStatistical = () => {
  ...
  const fetchData = async () => {
    ...
    const totalSalary = data.data.data.reduce((total, item) => {
      ...
      const salary = duration * user?.salary;
      return total + salary;
    }, 0);
        ...
        <div className="chamcong__wrapper">
          <div className="chamcong__heading">
            ...
            <div className="chamcong__heading-inputs">
              ...
              <label htmlFor="month">
                <span>Month</span>
                <input type="number" name='month' value={month ? month : 0} onChange={(e)=> setMonth(e.target.value)}/>
              </label>

              <label htmlFor="year">
                <span>Year</span>
                <input type="number" name='year' value={year ? year : 0} onChange={(e)=> setYear(e.target.value)}/>
              </label>
              <button onClick={()=>{
                setMonth()
                setYear()
              ...
              <div className="chamcong__content-table">
                {
                  dataTable ? dataTable.map((item, index) => {
                    ...
                    let date = `${new Date(item?.check_in).getDate()}/${new Date(item?.check_in).getMonth() + 1}/${new Date(item?.check_in).getFullYear()}`
                    ...
                    return(
                      <div className="chamcong__item" key={index}>
                        ...
                        <div className="chamcong__item-total chamcong__heading-item">
                          {Number((timeDifferenceHours * user?.salary).toFixed(0)).toLocaleString("en-US")}
                        </div>
                      </div>
                    )
                  }) : null
